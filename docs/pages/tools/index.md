...
#### Virtual Cell Atlas
Arc Institute launched the [*Arc Virtual Cell Atlas*](https://arcinstitute.org/tools/virtualcellatlas), a growing resource for computation-ready single-cell measurements, starting with data from over 300 million cells. The aim of this atlas is to enable researchers to train  machine learning models that can better capture the complexity of cellular behavior across diverse contexts. Arc scientists developed AI agents to automatically identify and curate single-cell datasets from public repositories, processing them through a standardized pipeline that ensures consistency across different experimental sources. This uniform approach enables better integration between different types of single-cell data and provides researchers with a dataset optimized and tailored for virtual cell model pretraining.
